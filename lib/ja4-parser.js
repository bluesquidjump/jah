...
const JA4Parser = {
  ...
  detectType(hash) {
    ...
    // Test JA4 patterns first (more specific due to underscores and structured prefixes)
    const ja4Types = ['JA4', 'JA4S', 'JA4H', 'JA4X', 'JA4SSH', 'JA4T', 'JA4TS'];
    for (const type of ja4Types) {
      if (this.patterns[type] && this.patterns[type].test(trimmed)) {
        return type;
      }
    }
  ...
  /**
   * Check if a type is a file hash type
  ...
  /**
   * Detect file hash type based on hex string length
   * @param {string} str - The hex string to check
   * @returns {string|null} - 'SHA256', 'SHA1', 'MD5', or null
   */
  detectFileHashType(str) {
    if (!str || typeof str !== 'string') return null;
    const trimmed = str.trim();
    if (!/^[a-f0-9]+$/i.test(trimmed)) return null;

    if (trimmed.length === 64) return 'SHA256';
    if (trimmed.length === 40) return 'SHA1';
    if (trimmed.length === 32) return 'MD5';
    return null;
  },

  /**
   * Parse a file hash and return structured data
   * @param {string} hash - The file hash
   * @returns {object|null}
   */
  parseFileHash(hash) {
    const type = this.detectFileHashType(hash);
    if (!type) return null;

    return {
      type,
      raw: hash.trim().toLowerCase(),
      isFileHash: true,
      components: {
        hashAlgorithm: type,
        hashValue: hash.trim().toLowerCase(),
  ...
  parse(hash) {
    ...
    switch (type) {
      ...
      case 'SHA1':
      case 'SHA256':
        return this.parseFileHash(trimmed);
      default:
